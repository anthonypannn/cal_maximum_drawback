...
4. 后续最大回撤 = 在上述观察区间内，按累计峰值法计算的最大回撤
"""
...
from calculate_drawdown import calculate_drawdown, find_max_drawdown_period
...
def analyze_single_index_below_yearline(
    ...
    for event_no, start_idx in enumerate(break_indices, start=1):
        ...
        drawdown_series, _ = calculate_drawdown(price_series)
        max_drawdown, dd_start_date, dd_end_date, dd_duration = find_max_drawdown_period(
            price_series, drawdown_series
        )
